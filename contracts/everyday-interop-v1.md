...
- `events` owns event-specific invitations, RSVP state, bring commitments, and setup/general/cleanup work.
...
- `eventworkboard://add?source=inventory&sourceId=<item-id>&label=<item-name>` stages an Inventory item as something to bring to the currently selected event.
- `eventworkboard://import?data=<encoded-event-snapshot>` stages a shared event for explicit user acceptance; accepting it creates a copied local event rather than merging or overwriting existing state.
